...
public final class P2cEwmaLoadBalancer implements LoadBalancer {
    ...
    @Override
    public ServiceInstance select(List<LoadBalanceContext> candidates) {
        ...
        return score(first) <= score(second) ? first.instance() : second.instance();
    }

    private static double score(LoadBalanceContext context) {
        long latencyNanos = Math.max(context.ewmaLatencyNanos(), 1_000_000L);
        return latencyNanos * (context.inflight() + 1.0) / context.instance().weight();
    }
}
