...
public final class PeachRpcServer implements AutoCloseable {
    ...
    private final ConcurrentMap<Integer, ServiceBinding> bindings = new ConcurrentHashMap<>();
    private final List<ServiceInstance> configuredInstances = new CopyOnWriteArrayList<>();
    ...
    private PeachRpcServer(Builder builder) {
        ...
        this.transport = Objects.requireNonNull(builder.transport, "transportServer");
        this.codecs = Objects.requireNonNull(builder.codecs, "codecRegistry");
        this.defaultCodec = codecs.defaultCodec();
        this.bindEndpoint = Objects.requireNonNull(builder.bind, "bindEndpoint");
    ...
    public PeachRpcServer registerService(
            Class<?> api, Object implementation, String version, String group) {
        if (state.get() != State.NEW) {
            throw new IllegalStateException("Services can only be registered before server start");
        }
        ...
        ServiceBinding previous = bindings.putIfAbsent(
                serviceId, new ServiceBinding(api, implementation));
        if (previous != null) {
            throw new IllegalStateException("Service id collision: " + serviceId);
        }
        configuredInstances.add(new ServiceInstance(
                UUID.randomUUID().toString(), key, bindEndpoint, 100, Map.of()));
        return this;
    }
    ...
    public CompletionStage<RpcEndpoint> start() {
        if (!state.compareAndSet(State.NEW, State.STARTING)) {
            return CompletableFuture.failedFuture(
                    new IllegalStateException("RPC server has already been started or closed"));
        }

        CompletableFuture<RpcEndpoint> started = new CompletableFuture<>();
        transport.start(bindEndpoint, this::handle).whenComplete((endpoint, transportError) -> {
            ...
            registerConfiguredServices(endpoint).whenComplete((ignored, registryError) -> {
                if (registryError != null) {
                    rollbackRegisteredServices(endpoint).whenComplete(
                            (rollbackIgnored, rollbackError) -> {
                                state.set(State.CLOSED);
                                transport.close();
                ...
                LOGGER.info("Peach RPC server started at {}", endpoint.authority());
    ...
    private CompletionStage<Void> registerConfiguredServices(RpcEndpoint endpoint) {
        List<CompletableFuture<Void>> registrations = new ArrayList<>();
        for (ServiceInstance configured : configuredInstances) {
            registrations.add(registry.register(runtimeInstance(configured, endpoint))
                    .toCompletableFuture());
        }
        return CompletableFuture.allOf(registrations.toArray(CompletableFuture[]::new));
    }

    private CompletionStage<Void> rollbackRegisteredServices(RpcEndpoint endpoint) {
        List<CompletableFuture<Void>> rollbacks = new ArrayList<>();
        for (ServiceInstance configured : configuredInstances) {
            CompletableFuture<Void> rollback = registry
        ...
        return CompletableFuture.allOf(rollbacks.toArray(CompletableFuture[]::new));
    }

    private CompletionStage<byte[]> handle(RpcEndpoint remote, byte[] rawFrame) {
        RpcFrame request;
        try {
        ...
        if (state.get() != State.STARTED) {
            return CompletableFuture.completedFuture(
                    frameworkError(request, RpcStatus.UNAVAILABLE, "Server is not ready"));
        }
        ...
        try {
            ...
            return CompletableFuture.completedFuture(
                    frameworkError(request, RpcStatus.BAD_REQUEST, "Invalid deadline"));
        }
        if (deadline > 0 && System.currentTimeMillis() > deadline) {
            return CompletableFuture.completedFuture(
                    frameworkError(request, RpcStatus.DEADLINE_EXCEEDED, "Deadline exceeded"));
        }

        ServiceBinding binding = bindings.get(request.serviceId());
        if (binding == null) {
            return CompletableFuture.completedFuture(
                    frameworkError(request, RpcStatus.SERVICE_NOT_FOUND, "Service not found"));
        }

        RpcCodec requestCodec;
        try {
            requestCodec = codecs.require(request.codec());
            ...
            return CompletableFuture.completedFuture(
                    response(
                            ...
                            defaultCodec.encode(RpcResultPayload.failure(
                                    RpcException.class.getName(), "Unsupported codec"))));
        }
        ...
        if (!admission.tryAcquire()) {
            return CompletableFuture.completedFuture(
                    response(
                            ...
                            requestCodec.encode(RpcResultPayload.failure(
                                    RpcException.class.getName(), "Server overloaded"))));
        }

        CompletableFuture<byte[]> result = new CompletableFuture<>();
        executor.submit(() -> execute(request, binding, requestCodec, result));
        return result;
    }
    ...
    private void execute(
        ...
        try {
            RpcInvocationPayload invocation = codec.decode(
                    request.payload(), RpcInvocationPayload.class);
            Object value = binding.invoke(request.methodId(), invocation.arguments());
            ...
            result.complete(response(
                    ...
                    codec,
                    RpcStatus.OK,
                    codec.encode(RpcResultPayload.success(value))));
        } catch (NoSuchMethodException error) {
            result.complete(response(
                    request,
                    codec,
                    RpcStatus.METHOD_NOT_FOUND,
                    codec.encode(RpcResultPayload.failure(
                            RpcException.class.getName(), "Method not found"))));
            ...
            result.complete(response(
                    ...
                    codec.encode(RpcResultPayload.failure(
                            error.getClass().getName(), "Remote service invocation failed"))));
        } finally {
            admission.release();
        }
    }

    private byte[] frameworkError(RpcFrame request, RpcStatus status, String message) {
        RpcCodec codec;
        try {
            codec = codecs.require(request.codec());
        ...
        return response(
                ...
                codec,
                status,
                codec.encode(RpcResultPayload.failure(RpcException.class.getName(), message)));
    }

    private static byte[] response(
            RpcFrame request, RpcCodec codec, RpcStatus status, byte[] payload) {
    ...
    private static ServiceInstance runtimeInstance(
            ServiceInstance configured, RpcEndpoint endpoint) {
    ...
    public void close() {
        ...
        if (endpoint != null) {
            for (ServiceInstance configured : configuredInstances) {
                try {
                    registry.unregister(runtimeInstance(configured, endpoint))
                            .toCompletableFuture()
                            .join();
                } catch (RuntimeException error) {
                    LOGGER.warn(
